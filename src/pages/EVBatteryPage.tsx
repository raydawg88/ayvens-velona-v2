    ...
    <div className="pb-16">
      ...
      >
        ...
        <p className="text-gray-500">
          {formatNumber(data.totalEvs)} electric vehicles monitored in real-time
        </p>
      </motion.div>
      ...
      >
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <p className="text-sm text-gray-500 mb-1">Total EVs</p>
          <p className="text-3xl font-display font-bold text-serene-900 tabular-nums">{formatNumber(data.totalEvs)}</p>
          <p className="text-sm text-gray-400 mt-1">across portfolio</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <p className="text-sm text-gray-500 mb-1">Avg Battery Health</p>
          <p className="text-3xl font-display font-bold text-emerald-600 tabular-nums">{formatPercent(data.avgBatteryHealth)}</p>
          <p className="text-sm text-gray-400 mt-1">{Math.round(data.avgCurrentRange)}/{data.avgRatedRange} km range</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <p className="text-sm text-gray-500 mb-1">Flagged Vehicles</p>
          <p className="text-3xl font-display font-bold text-red-600 tabular-nums">{data.vehiclesFlagged}</p>
          <p className="text-sm text-gray-400 mt-1">{criticalCount} critical, {watchCount} watch</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <p className="text-sm text-gray-500 mb-1">Replacement Cost Avoided</p>
          <p className="text-3xl font-display font-bold text-emerald-600 tabular-nums">{formatCurrency(data.replacementCostAvoided)}</p>
          <p className="text-sm text-gray-400 mt-1">via proactive monitoring</p>
        </div>
      </motion.div>
        ...
        {data.insights.map((insight, i) => {
          ...
          return (
            <div key={i} className={cn(
              ...
              isRisk ? 'bg-red-50 border-red-100' : 'bg-emerald-50 border-emerald-100'
            )}>
              ...
              <button className={cn('flex items-center gap-1 text-sm font-semibold transition-colors', isRisk ? 'text-red-700 hover:text-red-800' : 'text-emerald-700 hover:text-emerald-800')}>
                {isRisk ? 'View replacement plan' : 'View charging optimization'} <ChevronRight className="w-4 h-4" />
              </button>
            </div>
          )
        })}
      ...
      {/* ================================================================= */}
      {/* CHARTS: Health Distribution + Degradation Trend */}
      {/* ================================================================= */}
      <motion.section
        className="mb-14"
        initial={{ opacity: 0, y: 8 }}
      ...
      >
        <h2 className="text-2xl font-display font-bold text-serene-900 mb-5">Battery Health</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            ...
            <p className="text-sm text-gray-400 mb-5">Average vs worst battery health — 12 months</p>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
      ...
      >
        <div className="flex items-end justify-between mb-5">
          <h2 className="text-2xl font-display font-bold text-serene-900">Vehicle Details</h2>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {data.vehicles.map((v) => {
                  ...
                  const rangeLoss = v.ratedRange - v.currentRange
                  const rangePct = Math.round((v.currentRange / v.ratedRange) * 100)
                  ...
                  return (
                    <tr key={v.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-right">
                        ...
                        <p className="text-xs text-gray-400">-{rangeLoss} km ({100 - rangePct}% loss)</p>
                      </td>
